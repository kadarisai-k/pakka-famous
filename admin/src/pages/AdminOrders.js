...
const AdminOrders = () => {
  ...
  useEffect(() => { if (!authLoading) fetchOrders(); }, [statusFilter, dateFilter, startDate, endDate, page, authLoading]);
              ...
              <tbody>
                {orders.length === 0 ? (
                  ...
                  <tr key={order._id}>
                    ...
                    <td><span style={{ fontSize:'11px', background:'#f0f0f0', padding:'2px 8px', borderRadius:'4px' }}>{order.paymentMethod}</span></td>
                    <td>
                    ...
                    <td><Link to={`/orders/${order._id}`} className="btn btn-sm btn-outline-secondary"><FaEye /></Link></td>
                  </tr>
                ))}
              </tbody>
            </table>
